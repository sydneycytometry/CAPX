...
###################################################### 2. USER INPUT - DATA PREPARATION ###################################################### 
    ...
    ### 2.1 - Specify options
        ...
        data.name <- "BM_20180517_1500"   # suggest name is sampletype_date_time (e.g. liver_20180203_1400)
        
        #remove.keywords     <- 1   # inactive
        #remove.duplicates   <- 1   # inactive
        #arcsinh.transform   <- 1   # inactive
...
###################################################### 3. USER INPUT - LINE BY LINE ###################################################### 
    ...
    ### 3.1 - Read files into a list of dataframes (run all of 2.2, no user input)
        ...
        ## Specfiy file type (".csv" or ".fcs")
        File_Type <- ".csv" 
        
        ## Check the list of files
        FileNames <- list.files(path=PrimaryDirectory, pattern = File_Type) # Generate list of files
    ...
    ### 3.2 - OPTIONAL - Remove troubleshom columns (if required) and add new columns for SampleName and SampleNo
        ...
        #}
        #
        #length(names(DataList[[1]])) # check length of each
        #names(DataList[[1]]) # check for inconsistent columns in all samples
    ...
    ### 3.3 - IF REQUIRED - Arcsinh transformation (data may have already been transformed, SKIP IF NOT REQUIRED)        
        ...
        #DataList                                     # check the data has been converted 
        ...
        # Specify column numbers to be transformed - e.g. c(11, 23, 10)] to include the markers corresponding to the column numbers 11, 23, 10. Alternatively, e.g. c(1:32) to include all 32 columns
        ...
        asinh_scale <- 15                    # for CyTOF choose between 5 and 15, for flow you will need > 200.
        
        #var <- c("X148Nd_CD28", "X146Nd_CD8A")
    ...
    ### 3.5 - OPTIONAL - Add 'GroupNo' and 'GroupName' to each (required if generating 'group' files)
        ...
        Group_1_name <- "Mock D7" # enter the name for the first group     
        Group_2_name <- "WNV D7" # enter the name for the second group
    ...
    ### 3.6 - OPTIONAL - Specify downsample targets
        ...
        ## Create a list of cells per sample
        Cells.per.sample = list() # creates an empty list
        for(i in AllSampleNos){Cells.per.sample[[i]] <-  nrow(DataList[[i]])} # creates a list of the number of columns in each sample
        as.matrix(Cells.per.sample) # check that the number of columns in each sample is the same length
    ...
    ### 4.2 - Export data in individual files -- both .csv and .fcs format
        ...
        if(write.sep.files == 1){
          ...
          for (a in AllSampleNames) {
            ...
            ## write .csv
            write.csv(data_subset, file = paste(data.name, "_", a,".csv"))
