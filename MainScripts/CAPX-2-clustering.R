...
### Summary
    ...
    ## UPDATE -- IF function to determine if you want grouped files to be generated (if you have no group keywords, then choose not to)
...
###################################################### 2. USER INPUT - DATA PREPARATION ###################################################### 
    ...
    ### 2.1 - Specify options
        ...
        ## New name for data
        Data_name             <- "CAPX_20180517_1500"   # If one single file, what is the name of the file (not required if reading 'multi' files from a directory)
        Run_Groups            <- 1                # An option to write .csv and .fcs files of groups, as well as individuals samples. Turn OFF (0) if you have no group distinguishing keywords
        
        ## FlowSOM options
        Run_FlowSOM           <- 1                # Option to run FlowSOM. Enter 1 for yes, or 0 for no
        FlowSOM_k             <- 30               # Number of metaclusters to derive from FlowSOM (can also be selected automatically, but this often does not perform well)
        ...
        tSNE_dwnsmpl          <- "specific"       # can be "uniform" or "specific". 'uniform' means the entire dataset will be downsampled to a target total number of cells. 'specific' means each sample is downsampled to a specific value.
        tSNE_nsub             <- 70000            # ONLY used if tSNE_dwnsmpl = "specific". Specify the total downsample size for the data run in tSNE.
    ...
    ### 2.2 - Read files into a list of dataframes (run all of 2.2 at once, no user input)
        ...
        ## Loop for single files (run all of 2.2 at once)
        if(FileNos == "single"){
          
          if(File_Type == ".csv"){
            file.single <- list.files(path=PrimaryDirectory, pattern = ".csv")
            data <- read.csv(file.single[1])
          }
        ...
        ## Loop for multiple files (run all of 2.2 at once)
        if(FileNos == "multi"){
        ...
        ## Assign keywords for sample identification
        AllSampleNames <- unique(data$SampleName)
        AllSampleNos <- unique(data$SampleNo)
        
        ## Assign keywords for group identification (ONLY if writing 'group' .csv and .fcs files)
        AllGroupNames <- unique(data$GroupName)
        AllGroupNos <- unique(data$GroupNo)
    ...
    ### 2.3 - Choose markers for clustering
        ...
        ## Check which columns are being EXCLUDED!    
        ColumnNames[-ClusteringColNos]
        
        # Selected columns get used for clustering in both FlowSOM and tSNE. Can be modified if desired.
        
        
    ### 2.4 - Downsample targets for tSNE analysis
        
        ## Check individual samples for number of rows (i.e. number of cells)
        data$SampleName[1]
        sample.subset <- subset(data, SampleName == data$SampleName[1])
        nrow(sample.subset)
        
        ## Create a list of the number of rows 'cells' in each sample 
        SampleNames <- unique(data$SampleName) # unique sample names
        SampleNames
        grp.row.list = list()
        
        for(i in c(1:length(SampleNames))){
          grp.row.list[i] <- nrow(subset(data, SampleName == data$SampleName[i]))
        }
        ...
        sum(DownSampleTargets) # sum of all targets
...
###################################################### 3. DATA PREP ######################################################         
    ...
    ### 3.1. Save a copy of 'data' and a list of columns for clustering
        ...
        write.csv(x = data, file = data_csv)
...
###################################################### 4. FlowSOM ######################################################         
      ...
      if(Run_FlowSOM == 1) {
        ...
        write.csv(data, file = paste(paste0(Data_name), "_with_FlowSOM.csv", sep = ""))
        ...
        if(Run_Groups == 1){
            for(a in AllGroupNames){
              ...
              ## write .csv
              write.csv(data_subset, file = paste(Data_name, "_", a, "_with_FlowSOM", ".csv"))
    ...
    if(Run_tSNE == 1){
      ...
      if(tSNE_dwnsmpl == "specific"){
        ...
        for (i in AllSampleNos) {
          ...
          data.temp <- data[data$SampleNo == i,]
          data.temp
          nrow(data.temp)
          set.seed(tSNE_nsub_seed)
      ...
      write.csv(x = data_subsampled, file = paste0(Data_name, "_with_tSNE", ".csv"))
      ...
      if(Run_Groups == 1){
          for(a in AllGroupNames){
            data_subset <- subset(data_subsampled, GroupName == a)
            dim(data_subsampled)
            
            ## write .csv
            write.csv(data_subset, file = paste(Data_name, "_", a, "_with_tSNE.csv", sep = "")   )
      ...
      for (a in AllSampleNames) {
        ...
        data_subset <- subset(data_subsampled, SampleName == a)
        dim(data_subset)
...
###################################################### 6. tSNEplots ######################################################  
    ...
    if(Run_tSNEplots == 1){    
      ...
      PlotFiles <- list.files(path=PlotDirectory, pattern = ".csv")
      PlotFiles
