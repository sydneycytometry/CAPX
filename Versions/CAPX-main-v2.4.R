### Cytometry Analysis Pipeline for Complex Datasets (CAPX) v2.2
...
###################################################### 1. INSTALL AND-OR LOAD PACKAGES ###################################################### 
    ...
    ### 1.1 - Install packages (if not already installed) from CRAN
            ...
            ## From CRAN (required for clustering and tSNE)
                if(!require('SofieVG/FlowSOM')) {devtools::install_github("SofieVG/FlowSOM")} # for running FlowSOM ### POSSIBLY INSTALL FROM GITHUB
...
###################################################### 2. DATA INPUT and PREPARATION ###################################################### 
    ...
    ### 2.1 - Read data into workspace
            ...
            ## File details
                ...
                data.name               <- "Experiment_Name"    # a new name for the data - suggest name is sampletype_date_time (e.g. liver_20180203_1400)
...
###################################################### 3. USER INPUT - CLUSTERING AND DIMENSIONALITY REDUCTION PREFERENCES ###################################################### 
    ...
    ### 3.3 - If required, Arcsinh transformation     
            ...
            ## Perform transform (if selected in preferences, otherwise transformation will not run)
                ...
                #      x <- c(2,3)
                #
                #      #subset(data, select = col.names.SCALE)
                #      
                #      data[,c(2,3)]
                #      data[,x] ########## NEEDS TO BE A NUMBER
                #      
                #    
                #      data[,col.nos.scale]
                #      data[,col.names.SCALE]
                #      
                #      
                #      asinh_scale <- 5
    ...
    ### 3.4 - Write .csv and .fcs files of pre-processed samples   
            ...
            write.group.files       <- 1              # Do you also want to write one file for each group? (requires use.groups = 1) No = 0, Yes = 1
